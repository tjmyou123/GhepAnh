# Ky so dist\GhepAnh.exe + dist\ghep.exe bang chung chi tu ky "CN=Datpro09".
...
# 1) Tim chung chi ky code CN=Datpro09 trong kho ca nhan; chua co thi tao (han 5 nam)
$cert = Get-ChildItem Cert:\CurrentUser\My -CodeSigningCert |
    Where-Object { $_.Subject -eq "CN=Datpro09" -and $_.NotAfter -gt (Get-Date) } |
    Sort-Object NotAfter -Descending | Select-Object -First 1
if (-not $cert) {
    Write-Host "Dang tao chung chi tu ky CN=Datpro09..."
    $cert = New-SelfSignedCertificate -Type CodeSigningCert -Subject "CN=Datpro09" `
        -FriendlyName "Datpro09 Code Signing" -CertStoreLocation Cert:\CurrentUser\My `
...
Export-Certificate -Cert $cert -FilePath "$PSScriptRoot\Datpro09.cer" -Force | Out-Null
...
Write-Host "Luu y: chung chi TU KY nen may chua cai Datpro09.cer se bao 'UnknownError'/SmartScreen."
